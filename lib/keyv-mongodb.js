...
module.exports = class KeyvMongoDB {
	constructor (options = { collection: 'keyv', url: null, mongoOptions: { useNewUrlParser: true }, db: null, dbPromise: null, mongooseConnection: null }) {
		...
		if (options.url) {
			MongoClient.connect(options.url, options.mongoOptions || { useNewUrlParser: true }, (err, client) => {
	...
	setCollection (collection) {
		...
		this.mongo = [ 'updateOne', 'findOne', 'deleteMany', 'deleteOne' ].reduce((obj, method) => {
	...
	async set (key, value, ttl) {
		...
		return this.mongo.updateOne({ key }, { $set: { key, value, expiresAt } }, { upsert: true });
	}
